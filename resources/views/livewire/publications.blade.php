<div x-data="{ sidebarOpen: false, menuOpen: false }" x-cloak class="h-full z-10 bg-gray-50">
                      ...
                      <li class="relative">
                          <a  href="{{ route('menus.index', ['id' => $menu->id]) }}"
                            ...
                            class="nav-link font-semibold 
                                {{ Request::segment(2) == $menu->slug ? 'text-emerald-500 after:bg-emerald-500' : 'text-gray-500 after:bg-gray-500' }} 
                  ...
                  <ul role="list" class="flex flex-1 flex-col gap-y-7">
                    <li>
                      <ul role="list" class="-mx-2 space-y-1">
                        ...
                        <form wire:submit.prevent="filterItems" class="md:block">
                          <li class="px-2 mt-2">
                              <label for="city" class="block text-sm font-medium leading-6 text-gray-900">Ville</label>
                              <div>
                                  <select wire:model="selectedCity" class="block w-full shadow-sm rounded-md border-0 py-1.5 ring-gray-300 ring-1  focus:ring-2 ring-inset focus:ring-emerald-500 sm:max-w-xs sm:text-sm sm:leading-6">
                          ...
                          <li class="px-2 mt-2">
                              ...
                              <div class="relative rounded-md shadow-sm">
                                  <input wire:model="selectedDateStart" id="publicationDatePickerr" type="date" class="form-input block w-full rounded-md border-0 py-1.5 ring-gray-300 ring-1 ring-inset focus:ring-2 focus:ring-emerald-500 sm:text-sm sm:leading-6">
                              </div>
                          </li>
                          <li class="px-2 mt-2">
                              <label for="type" class="block text-sm font-medium leading-6 text-gray-900">Type</label>
                              <div>
                                  <select wire:model="selectedType" id="type" class="block w-full shadow-sm rounded-md border-0 py-1.5 ring-gray-300 ring-1 focus:ring-2 ring-inset focus:ring-emerald-500 sm:max-w-xs sm:text-sm sm:leading-6">
                          ...
                          <li class="px-2 mt-2">
                              ...
                              <div>
                                  <select wire:model="selectedPrice" id="price" class="block w-full shadow-sm rounded-md border-0 py-1.5 ring-gray-300 ring-1 focus:ring-2 ring-inset focus:ring-emerald-500 sm:max-w-xs sm:text-sm sm:leading-6">
                          ...
                          <li class="px-2 mt-2">
                              <label for="sort" class="block text-sm font-medium leading-6 text-gray-900">Triér par :</label>
                              <div>
                                  <select wire:model="selectedSort" id="sort" class="block w-full shadow-sm rounded-md border-0 py-1.5 ring-gray-300 ring-1 focus:ring-2 ring-inset focus:ring-emerald-500 sm:max-w-xs sm:text-sm sm:leading-6">
                                      <option value="ds">Horaire</option>
                                      <option value="pricemin">Prix ascendant</option>
                                      <option value="pricemax">Prix descendant</option>
                                  </select>
                              </div>
                          </li>
                          <li class="px-2 mt-4">
          ...
          <div class="flex grow flex-col gap-y-5 shadow-lg  overflow-y-auto mt-2 px-3">
            <nav class="flex flex-1 flex-col">
              <div class="space-y-[10px] px-1">
                ...
                <form wire:submit.prevent="loadItems" class="md:block">
                  <div>
                    <!-- Search Field -->
                    <div class="w-full max-w-lg lg:max-w-xs">
                      ...
                      <div class="relative">
                        ...
                        <input id="search" name="search" class="block w-full rounded-md border-0 bg-transparent py-1.5 pl-10 pr-3 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-emerald-500 sm:text-sm sm:leading-6" placeholder="Recherche" type="search" value="{{ old('search') }}">
                    </div>
                    </div>
                    ...
                    <div>
                      <x-input-label for="birth" :value="__('')" />
                      <x-input-flatpickr wire:model="selectedDate" id="birth" class="flatpickr block bg-transparent mt-3 py-1.5 w-full" type="date" name="birth" placeholder="Séléctionnez une date"/>
                      <x-input-error :messages="$errors->get('birth')" class="mt-2" />
                    </div>
                      ...
                      <div class="border border-gray-300 rounded-md relative mt-6">
                        ...
                        <div class="block mt-3">
                          <label for="physical" class="inline-flex items-center">
                            <input wire:model="selectedType" id="physical" type="radio" class="ml-2 rounded bg-transparent border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" name="type" value="physical">
                            <span class="ms-2 text-sm text-gray-600">{{ __('Présentielle') }}</span>
                          </label>
                        </div>
                        <div class="block mt-2">
                          <label for="virtual" class="inline-flex items-center">
                            <input wire:model="selectedType" id="virtual" type="radio" class="ml-2 rounded bg-transparent border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" name="type" value="virtual">
                      ...
                      <div class="border border-gray-300 rounded-md relative mt-6">
                        ...
                        <div class="block mt-3">
                          <label for="free" class="inline-flex items-center">
                            <input wire:model="selectedPrice" id="free" type="radio" class="ml-2 rounded bg-transparent border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" name="price" value="gratuit">
                            <span class="ms-2 text-sm text-gray-600">{{ __('Gratuit') }}</span>
                          </label>
                        </div>
                        <div class="block mt-2">
                          <label for="paid" class="inline-flex items-center">
                            <input wire:model="selectedPrice" id="paid" type="radio" class="ml-2 rounded bg-transparent border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" name="price" value="payant">
                      ...
                      <div class="border border-gray-300 rounded-md relative mt-6">
                        ...
                        <div class="block mt-3">
                          <label for="sortByDate" class="inline-flex items-center">
                            <input wire:model="sortBy" id="sortByDate" type="radio" class="ml-2 rounded bg-transparent border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" name="sort" value="ds">
                            <span class="ms-2 text-sm text-gray-600">{{ __('Horaire') }}</span>
                          </label>
                        </div>
                        <div class="block mt-2">
                          <label for="sortByPriceAsc" class="inline-flex items-center">
                            <input wire:model="sortBy" id="sortByPriceAsc" type="radio" class="ml-2 rounded bg-transparent border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" name="sort" value="pricemin">
                            <span class="ms-2 text-sm text-gray-600">{{ __('Prix ascendant') }}</span>
                          </label>
                        </div>
                        <div class="block mt-2">
                          <label for="sortByPriceDesc" class="inline-flex items-center">
                            <input wire:model="sortBy" id="sortByPriceDesc" type="radio" class="ml-2 rounded bg-transparent border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" name="sort" value="pricemax">
                  ...
                  <button type="submit" class="bg-emerald-500 text-white px-2 py-1 mt-2 text-xs md:text-sm font-semibold rounded-md">
                    Mettre à jour
                </button>
                </form>
                    ...
                    <div class="-mt-2">
                      @if(empty($items))
                        ...
                        @foreach($items as $item)
                          {{-- debut du post --}}
                          <article class="flex flex-col lg:flex-row z-0 pb-6 my-5 md:pt-6 pl-0 sm:px-2 drop-shadow-md rounded-lg lg:px-4 bg-white relative">
      ...
      Alpine.data('singleSelectCombobox', (selectedCity, cities) => ({
          search: '',
          open: false,
          ...
          filterOptions() {
              this.filteredOptions = this.cities.filter(city => city.name.toLowerCase().includes(this.search.toLowerCase()));
          },
          selectOption(option) {
              this.selectedCity = option.id;
              this.selectedCityObject = option;
          ...
          init() {
              this.selectedCityObject = this.cities.find(city => city.id === this.selectedCity);
              if (this.selectedCityObject) {
                  this.search = this.selectedCityObject.name;
              }
              this.$watch('search', value => {
                  if (!this.open && value !== (this.selectedCityObject ? this.selectedCityObject.name : '')) {
                      this.open = true;
                      this.filterOptions();
                  }
              });
        ...
        <script>
          function singleSelectCombobox(selectedCity, cities) {
              return {
                  search: '',
                  open: false,
                  cities: cities,
          ...
          document.addEventListener('livewire:load', function () {
              Livewire.on('resetFields', () => {
                  const comboboxComponent = document.getElementById('cityCombobox')._x_dataStack[0];
