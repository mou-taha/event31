<div class="mt-4">
                ...
                <form wire:submit.prevent="store" class="divide-y divide-gray-200 lg:col-span-12">
                        ...
                        <div class="mt-2 flex flex-col lg:flex-row">
                                ...
                                <div class="grid grid-cols-12 gap-6">
                                    <div class="col-span-12 sm:col-span-6">
                                        ...
                                        <div class="flex rounded-md shadow-sm">
                                            <x-text-input wire:model="name" placeholder="Nom d'organisme" id="name" name="name" type="text" class="mt-1 block w-full" autofocus  />
                                    ...
                                    <div class="col-span-12 sm:col-span-6">
                                        ...
                                        <div class="flex rounded-md shadow-sm">
                                            <x-text-input wire:model="email" placeholder="Email d'organisme" id="email" name="email" type="email" class="mt-1 block w-full" autofocus  />
                                    ...
                                    <div class="col-span-12 sm:col-span-6 mt-1">
                                        ...
                                        <x-input-error :messages="$errors->get('selectedCity')" />
                                    </div>
                                    <div class="col-span-12 sm:col-span-6">
                                        <label for="address" class="block text-sm font-medium leading-6 text-gray-900">Adresse</label>
                                        <div class="flex rounded-md shadow-sm">
                                            <x-text-input wire:model="address" id="address" placeholder="Ajouter une adresse" name="address" type="text" class="mt-1 block w-full" autofocus  />
                                            <x-input-error class="mt-2" :messages="$errors->get('address')" />
                                        </div>
                                    </div>
                                    <div class="col-span-12 sm:col-span-6">
                                        <label for="map" class="block text-sm font-medium leading-6 text-gray-900">Maps</label>
                                        <div class="flex rounded-md shadow-sm">
                                            <x-text-input wire:model="map" id="map" placeholder="Lien google maps" name="map" type="text" class="mt-1 block w-full" autofocus  />
                                        ...
                                        <div>
                                            @foreach($phones as $index => $phone)
                                                <div class="col-span-12 sm:col-span-6 relative mb-2">
                                                    <x-text-input wire:model="phones.{{ $index }}.number" id="phone_{{ $index }}" name="phone_{{ $index }}" type="text" placeholder="Numéro de téléphone" class="block w-full pr-10" />
                                                    <button type="button" wire:click.prevent="removePhone({{ $index }})" class="absolute right-2 top-1/2 transform -translate-y-1/2 text-red-500">X</button>
                                                </div>
                                            @endforeach
                                            <div class="col-span-12 sm:col-span-6">
                                                <button type="button" wire:click.prevent="addPhone" class="mt-2 text-blue-500">+ Ajouter un Numéro</button>
                                            </div>
                                        </div>
                                        <x-input-error :messages="$errors->get('phones.*.number')" />
                            ...
                            <div class="w-full mt-2 lg:w-1/3 lg:pl-6 lg:py-2">
                                <div class="relative flex flex-col items-center justify-center w-full h-32 border border-gray-300 rounded-lg bg-gray-50">
                                    ...
                                    @elseif ($logo)
                                        <img src="{{ Storage::url($logo) }}" alt="Current Logo" class="object-cover w-full h-full rounded-lg">
                                    @else
                                        <div class="flex items-center justify-center w-full h-full">
                                            <span class="text-sm text-gray-400">Logo d'organisme</span>
                                        </div>
                                    @endif
                                    <input wire:model="newLogo" type="file" id="logo" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                                </div>
                                <x-input-error class="mt-2" :messages="$errors->get('newLogo')" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="divide-y mt-2 divide-gray-200">
                        <div class="flex justify-end gap-x-3 px-4 py-4 sm:px-6">
                            <a  href="{{route('organisms')}}" class="inline-flex justify-center rounded-md bg-white px-5 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">Fermer</a>
    ...
    <div aria-live="assertive" class="pointer-events-none fixed inset-0 flex items-end px-4 py-6 sm:items-start sm:p-6">
        <div class="flex w-full flex-col items-center space-y-4 sm:items-end">
            ...
            >
            <a  href="{{route('organisms')}}" class="inline-flex justify-center rounded-md sr-only bg-white px-5 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">Retour</a>
    ...
    Livewire.on('updateSelectedCity', function (cityId) {
        const comboboxComponent = document.getElementById('cityCombobox')._x_dataStack[0];
        const city = comboboxComponent.cities.find(city => city.id === cityId);
        comboboxComponent.selectedCity = cityId;
        comboboxComponent.search = city ? city.name : '';
    });
});
    </script>
    <script>
