<div x-data="{ liked: @entangle('liked'), showLoginModal: false }" @show-login-modal.window="showLoginModal = true">
    <svg wire:click="toggleLike" :class="{ 'text-red-500': liked, 'text-[#888888]': !liked }" xmlns="http://www.w3.org/2000/svg" class="md:ml-2 mr-1 w-8 h-8 cursor-pointer" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 19.654l-.758-.685q-2.448-2.236-4.05-3.829q-1.602-1.592-2.529-2.808q-.926-1.217-1.295-2.201Q3 9.146 3 8.15q0-1.908 1.296-3.204Q5.592 3.65 7.5 3.65q1.32 0 2.475.675T12 6.288Q12.87 5 14.025 4.325T16.5 3.65q1.908 0 3.204 1.296Q21 6.242 21 8.15q0 .996-.368 1.98q-.369.985-1.295 2.202q-.927 1.216-2.52 2.808q-1.592 1.593-4.06 3.83zm0-1.354q2.4-2.17 3.95-3.716q1.55-1.547 2.45-2.685t1.25-2.015Q20 9.006 20 8.15q0-1.5-1-2.5t-2.5-1q-1.194 0-2.204.682q-1.01.681-1.808 2.053h-.976q-.818-1.39-1.818-2.063q-1-.672-2.194-.672q-1.48 0-2.49 1Q4 6.65 4 8.15q0 .856.35 1.734t1.25 2.015t2.45 2.675Q9.6 16.112 12 18.3m0-6.825"/>
    </svg>

    <!-- Modal -->
    <div x-show="showLoginModal" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-75">
        <div class="bg-white p-6 rounded shadow-lg text-center">
            <h2 class="text-2xl mb-4">You need to be logged in to like an event.</h2>
            <a href="{{ route('login') }}" class="bg-blue-500 text-white py-2 px-4 rounded">Login</a>
            <a href="{{ route('register') }}" class="bg-green-500 text-white py-2 px-4 rounded">Register</a>
            <button @click="showLoginModal = false" class="bg-gray-500 text-white py-2 px-4 rounded mt-4">Close</button>
        </div>
    </div>
    <script>
        document.addEventListener('livewire:load', function () {
    Livewire.on('showLoginModal', () => {
        window.dispatchEvent(new CustomEvent('show-login-modal'));
    });
});
    </script>
</div>
