    ...
    <div x-data="{ sidebarOpen: false, menuOpen: false }" x-cloak class="h-full z-10 bg-gray-50">
      <header class="bg-white fixed top-0 left-0 right-0 z-30">
          ...
          <div class="hidden md:flex pb-3 pt-3 shadow-sm justify-center">
              ...
              @endforeach
                  <li class="relative">
                      <a href="{{ route('blogs.index') }}" class="nav-link font-semibold {{ request()->routeIs('blogs.show') ? 'text-emerald-500' : 'text-gray-500' }} text-md relative transition ease-in-out duration-300 after:absolute after:bottom-0 after:left-0 after:w-0 after:h-px after:bg-gray-500 after:transition-all after:duration-300 hover:after:w-full">Blogs</a>
                  </li>
                  <!-- <li class="relative">
      ...
      <div class="bg-white">
        <div class="space-y-10 pt-36 md:space-y-16">
            <section class="bg-white dark:bg-gray-900">
                <div class="container px-6 pb-10 mx-auto">
                    <div class="lg:flex lg:-mx-6">
                        <div class="lg:w-3/4 lg:px-6">
                            <img class="object-cover object-center w-full h-80 xl:h-[28rem] rounded-xl" src="{{ str_starts_with($blog->thumbnail, 'http') ? $blog->thumbnail : asset('storage/' . $blog->thumbnail) }}" alt="{{ $blog->thumbnail }}">
            
                            <div>
                              <a href="{{ route('blogs.byCategory', ['category' => $blog->category]) }}">
                                <p class="mt-6 text-sm text-emerald-700 uppercase">{{ $blog->category->name }}</p>
                              </a>
                                <h1 class="max-w-lg mt-4 text-2xl font-semibold leading-tight text-gray-800 dark:text-white">
                                    {{ $blog->title }}
                                </h1>
                                <p class="mt-3">{!! nl2br(e($blog->content)) !!}</p>
                                <div class="md:flex items-end justify-between mt-2">
                                    <div>
                                    </div>
                                    <div class="justify-end">
                                        <a href="{{route('blogs.index')}}" class="flex justify-center w-64 text-center mt-4 py-2 px-7 md:mx-2 font-semibold bg-emerald-600 transition text-slate-50 rounded-lg">
                                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                              <path stroke-linecap="round" stroke-linejoin="round" d="M12 7.5h1.5m-1.5 3h1.5m-7.5 3h7.5m-7.5 3h7.5m3-9h3.375c.621 0 1.125.504 1.125 1.125V18a2.25 2.25 0 01-2.25 2.25M16.5 7.5V18a2.25 2.25 0 002.25 2.25M16.5 7.5V4.875c0-.621-.504-1.125-1.125-1.125H4.125C3.504 3.75 3 4.254 3 4.875V18a2.25 2.25 0 002.25 2.25h13.5M6 7.5h3v3H6v-3z" />
                                          </svg>
                                          <span class="flex">Retour</span>
                                      </a>
                        ...
                        <div class="mt-8 lg:w-1/4 lg:mt-0 lg:px-6">
                          <h2 class="max-w-2xl text-2xl font-bold tracking-tight text-gray-700 sm:text-3xl">Nos derniers blog</h2>
        
                            @foreach ($latestFive as $blog)
                            <hr class="my-6 border-gray-200 dark:border-gray-700"> 
                            <div>
                              @if ($blog->category)
                                <p class="text-emerald-700 capitalize">{{ $blog->category->name }}</p>
                              @endif
                                <a href="{{ route('blogs.show', ['blog' => $blog]) }}" class="block mt-2 font-medium text-gray-700 hover:underline hover:text-gray-500 ">
                                  {{ $blog->title }}
                                </a>
                            </div>
                            @endforeach
